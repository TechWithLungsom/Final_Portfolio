...
export function SkillsMarquee(){return <div className="skill-marquees"><div className="marquee-instruction">THE TOOLKIT <span>HOVER OR FOCUS TO PAUSE</span></div>{categories.map((row,i)=><div className="marquee-row" key={row.label}><h3>{row.label}</h3><div className="marquee-window" tabIndex={0} aria-label={`${row.label}: ${row.items.join(', ')}. Focus to pause animation.`}><div className={`marquee-track ${i%2?'reverse':''}`}>{[0,1].map(copy=><div className="marquee-set" key={copy} aria-hidden={copy===1}>{row.items.map(item=><span key={item}>{item}<i>✳</i></span>)}</div>)}</div></div></div>)}</div>}
